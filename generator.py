...
from database import get_thought_base_sync, save_thought_history_sync, get_all_thoughts_sync

def generate_thought():
    thought_base = get_thought_base_sync()
    # ... rest stays the same

def check_duplicate(new_thought):
    existing_thoughts = get_all_thoughts_sync()
    # ... rest stays the same
...
async def send_daily_thought():
    ...
    # Save to database
    thought_base_snapshot = get_thought_base_sync()
    thought_id = save_thought_history_sync(thought, thought_base_snapshot)
    # ... rest stays the same
